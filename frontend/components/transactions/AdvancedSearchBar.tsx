...
export function AdvancedSearchBar({ transactions, onFilter, onSetDateContext, initialQuery = '', accounts = [], extraChips = [], onRegisterClearAll, hideTriggers = false, onRegisterOpenSearch, onRegisterOpenFilter, onActiveFilterCountChange }: Props) {
    ...
    const pillS = (active: boolean): React.CSSProperties => ({
        ...
        background: active ? 'var(--accent)' : 'var(--glass-fill-1)',
    ...
    const inputS: React.CSSProperties = {
        padding: '7px 10px', background: 'var(--glass-fill-1)', border: '1px solid var(--glass-border)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
                ...
                <div style={{ position: 'relative', height: '28px', marginBottom: '12px' }}>
                    <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, height: '4px', background: 'var(--glass-fill-1)', borderRadius: '2px', transform: 'translateY(-50%)' }} />
                            ...
                            {allCategories.map(cat => {
                                ...
                                return (
                                    <button key={cat} type="button"
                                        ...
                                        onMouseEnter={e => (e.currentTarget.style.background = 'var(--glass-fill-2)')}
                        ...
                        {allTags.map(tag => {
                            ...
                            return (
                                <button key={tag} type="button"
                                    ...
                                    style={{ padding: '3px 10px', borderRadius: '999px', border: `1px solid ${active ? '#8b5cf6' : 'var(--border-subtle)'}`, background: active ? 'rgba(139,92,246,0.12)' : 'var(--glass-fill-1)', color: active ? '#8b5cf6' : 'var(--text-secondary)', fontSize: '12px', fontFamily: 'var(--font-body)', cursor: 'pointer', transition: 'all 0.15s' }}>
                                    #{tag}
                                </button>
                            );
                        })}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {!searchExpanded ? (
                    ...
                    <button type="button" onClick={() => { setSearchExpanded(true); setTimeout(() => inputRef.current?.focus(), 30); }}
                        ...
                        style={{
                            ...
                            background: 'var(--glass-fill-1)', border: '1px solid var(--glass-border)',
                    ...
                    <div
                        style={{
                            ...
                            background: 'var(--glass-fill-1)', border: '1px solid var(--accent)',
                ...
                <button type="button" onClick={() => setPanelOpen(o => !o)} title="Filters"
                    style={{
                        ...
                        background: panelOpen ? 'var(--accent-subtle)' : 'var(--glass-fill-1)',
            ...
            {(summaryChips.length > 0 || extraChips.length > 0) && (
                <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: '2px', alignItems: 'center' }}>
                    ...
                    {summaryChips.map((chip, i) => (
                        <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '3px 8px 3px 10px', borderRadius: '999px', flexShrink: 0, background: 'var(--glass-fill-1)', border: '1px solid var(--glass-border)', fontSize: '12px', color: 'var(--text-secondary)', fontFamily: 'var(--font-body)', fontWeight: 500, whiteSpace: 'nowrap' }}>
                            {chip.label}
            ...
            {savedViews.length > 0 && (
                <div ref={dotMenuRef} style={{ display: 'flex', gap: '6px', overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: '2px', alignItems: 'center' }}>
                    {savedViews.map(view => (
                        <div key={view.id} style={{ display: 'inline-flex', alignItems: 'center', background: 'var(--glass-fill-1)', border: '1px solid var(--glass-border)', borderRadius: '999px', fontSize: '12px', fontFamily: 'var(--font-body)', flexShrink: 0, overflow: 'visible', position: 'relative' }}>
                            {renameId === view.id ? (
            ...
            {saveModalOpen && mounted && createPortal(
                <div onClick={() => setSaveModalOpen(false)}
                    ...
                    <div onClick={e => e.stopPropagation()}
                        style={{ background: 'var(--glass-fill-1)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-xl)', padding: '24px', width: '100%', maxWidth: '360px', boxShadow: 'var(--shadow-modal)', animation: 'springIn 280ms cubic-bezier(0.34,1.56,0.64,1) both' }}>
                        ...
                        <input autoFocus type="text" placeholder="e.g. Weekend Splurges"
                            ...
                            style={{ width: '100%', padding: '10px 12px', background: 'var(--glass-fill-1)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-md)', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'var(--font-body)', outline: 'none', boxSizing: 'border-box', marginBottom: '16px', transition: 'border-color var(--transition-fast)' }}
                            onFocus={e => (e.target.style.borderColor = 'var(--accent)')}
                            onBlur={e => (e.target.style.borderColor = 'var(--border-subtle)')} />
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                            <button type="button" onClick={() => setSaveModalOpen(false)}
                                style={{ padding: '8px 16px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--glass-border)', background: 'var(--glass-fill-1)', color: 'var(--text-secondary)', fontSize: '13px', fontFamily: 'var(--font-body)', cursor: 'pointer' }}>
                                Cancel
                            </button>
                            <button type="button" onClick={saveView} disabled={!saveViewName.trim()}
                                style={{ padding: '8px 16px', borderRadius: 'var(--radius-sm)', border: 'none', background: saveViewName.trim() ? 'var(--accent)' : 'var(--glass-fill-1)', color: saveViewName.trim() ? 'white' : 'var(--text-muted)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-body)', cursor: saveViewName.trim() ? 'pointer' : 'not-allowed', transition: 'all 0.15s' }}>
                                Save
                            </button>
                        </div>
                    </div>
