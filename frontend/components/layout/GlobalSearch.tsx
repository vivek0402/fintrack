...
export function GlobalSearch() {
    ...
    const rowHover = (e: React.MouseEvent<HTMLElement>, enter: boolean) => {
        (e.currentTarget as HTMLElement).style.background = enter ? 'var(--bg-hover)' : 'transparent';
    };
    ...
    return (
        <>
            <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', padding: '8px 12px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', color: 'var(--text-muted)', fontSize: '0.8rem', cursor: 'pointer', marginBottom: '8px', fontFamily: 'var(--font-body)' }}
            >
                <Search size={14} />
                Search… (Ctrl+K)
            </button>
            ...
            {open && mounted && createPortal(
                <>
                    ...
                    <div style={{ position: 'fixed', top: '80px', left: '50%', transform: 'translateX(-50%)', width: '90%', maxWidth: '580px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', zIndex: 10000, overflow: 'hidden', boxShadow: 'var(--shadow-modal)', animation: 'springIn 260ms cubic-bezier(0.34,1.56,0.64,1) both' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
                            ...
                            {query
                                ...
                                : <span style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'var(--font-body)', background: 'var(--bg-alt)', border: '1px solid var(--border)', borderRadius: '4px', padding: '2px 6px' }}>ESC</span>
                            }
                        </div>
                        ...
                        <div style={{ maxHeight: '420px', overflowY: 'auto' }}>
                                ...
                                <>
                                    ...
                                    {topTransactions.map(tx => {
                                        ...
                                        return (
                                            <div key={tx.id}
                                                ...
                                                style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', cursor: 'pointer', borderBottom: '1px solid var(--border)' }}
                                    ...
                                    {results.length > 5 && (
                                        <button onClick={() => navigateToTransactions(query)}
                                            style={{ display: 'block', width: '100%', padding: '8px 16px', background: 'none', border: 'none', borderBottom: '1px solid var(--border)', cursor: 'pointer', color: 'var(--accent)', fontSize: '12px', fontFamily: 'var(--font-body)', textAlign: 'left', fontWeight: 500 }}
                                            onMouseEnter={e => rowHover(e, true)}
                                            onMouseLeave={e => rowHover(e, false)}>
                                ...
                                <>
                                    ...
                                    {topCategories.map(cat => (
                                        <div key={cat}
                                            ...
                                            style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 16px', cursor: 'pointer', borderBottom: '1px solid var(--border)' }}
                                            onMouseEnter={e => rowHover(e, true)}
                                            onMouseLeave={e => rowHover(e, false)}>
                            ...
                            {quickActions.map(action => (
                                <div key={action.label}
                                    ...
                                    onMouseEnter={e => rowHover(e, true)}
                                    onMouseLeave={e => rowHover(e, false)}>
                                    <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'var(--bg-alt)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'var(--text-secondary)' }}>
                                        {action.icon}
                                    </div>
