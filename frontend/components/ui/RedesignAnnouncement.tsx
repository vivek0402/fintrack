...
export function RedesignAnnouncement() {
    ...
    return createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)' }}>
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '460px', maxHeight: '90vh', overflowY: 'auto', animation: 'springIn 380ms cubic-bezier(0.34,1.56,0.64,1) both', boxShadow: 'var(--shadow-modal)' }}>

                {/* Hero */}
                <div style={{ padding: '28px 28px 20px', textAlign: 'center', borderBottom: '1px solid var(--border)' }}>
                    <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'color-mix(in srgb, var(--color-info) 15%, var(--bg-card))', border: '1px solid color-mix(in srgb, var(--color-info) 25%, transparent)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
                ...
                <div style={{ padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        {WHATS_NEW.map(item => (
                            <div key={item.title} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                                ...
                                <div>
                                    <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 1px', fontFamily: 'var(--font-head)' }}>{item.title}</p>
                    ...
                    <button type="button" onClick={handleDone}
                        style={{ width: '100%', padding: '13px', background: 'var(--accent)', border: 'none', borderRadius: 'var(--radius-md)', color: 'white', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-head)', transition: 'opacity 0.15s' }}
