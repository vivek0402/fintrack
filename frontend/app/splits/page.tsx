...
const inputSt: React.CSSProperties = { width: '100%', padding: '10px 14px', background: 'var(--bg-alt)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 10, fontSize: '0.875rem', fontFamily: 'var(--font-body)', outline: 'none' };
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '24px', animation: 'fadeUp 200ms ease forwards' }}>
                ...
                <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        ...
                        <button type="button" onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '8px 14px', background: 'var(--accent-light)', border: '1px solid var(--accent-border)', borderRadius: 'var(--radius-md)', color: 'var(--accent)', fontSize: '13px', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
                ...
                <div style={{ background: 'color-mix(in srgb, var(--color-info) 8%, var(--bg-card))', border: '1.5px solid color-mix(in srgb, var(--color-info) 20%, transparent)', borderRadius: 'var(--radius-lg)', padding: '14px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px' }}>
                        ...
                        <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--color-info)', fontFamily: 'var(--font-head)' }}>AI Parse Split</span>
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <input type="text" placeholder='"Dinner ₹2400 split 4 ways with Raj, Priya, Sam"'
                            ...
                            style={{ flex: 1, padding: '9px 12px', background: 'var(--bg-alt)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 9, fontSize: '13px', fontFamily: 'var(--font-body)', outline: 'none', transition: 'border-color var(--transition-fast)' }}
                            onFocus={e => (e.target.style.borderColor = 'var(--accent)')}
                            onBlur={e => (e.target.style.borderColor = 'var(--border)')}
                        />
                        <button type="button" onClick={handleNlParse} disabled={nlLoading || !nlInput.trim()}
                            style={{ padding: '9px 16px', background: 'var(--accent-light)', border: '1px solid var(--accent-border)', borderRadius: 9, color: 'var(--accent)', fontSize: '13px', fontWeight: 600, cursor: nlLoading || !nlInput.trim() ? 'not-allowed' : 'pointer', opacity: nlLoading || !nlInput.trim() ? 0.6 : 1, fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>
                            {nlLoading ? '…' : 'Parse'}
                        </button>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '40px 24px' }}>
                        ...
                        <p style={{ fontFamily: 'var(--font-head)', fontSize: '15px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 6px' }}>No splits yet</p>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {splits.map(split => {
                            ...
                            return (
                                <div key={split.id} style={{ background: 'var(--bg-card)', border: `1px solid ${settled ? 'color-mix(in srgb, var(--color-inc) 20%, transparent)' : 'var(--border)'}`, borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                                    {/* Header */}
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '14px 16px 10px', gap: '10px' }}>
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '4px' }}>
                                                <span style={{ fontFamily: 'var(--font-head)', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{split.description}</span>
                                        ...
                                        <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
                                            <button type="button" onClick={() => openEdit(split)} style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-muted)', cursor: 'pointer' }}
                                            ...
                                            <button type="button" onClick={() => handleDelete(split.id)} disabled={deletingId === split.id} style={{ width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-muted)', cursor: 'pointer', opacity: deletingId === split.id ? 0.5 : 1 }}
                                    ...
                                    {/* Participants */}
                                    <div style={{ borderTop: '1px solid var(--border)', padding: '8px 16px 10px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                        {split.participants.map((p: any, i: number) => (
                                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <div style={{ width: 28, height: 28, borderRadius: '50%', background: p.settled ? 'color-mix(in srgb, var(--color-inc) 12%, transparent)' : 'var(--bg-alt)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, color: p.settled ? 'var(--color-inc)' : 'var(--text-secondary)', flexShrink: 0, fontFamily: 'var(--font-head)' }}>
            ...
            <Modal isOpen={showModal} onClose={closeModal} title={editingSplit ? 'Edit Split' : 'New Split'} maxWidth="500px">
                ...
                <GCard style={{ marginBottom: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                        ...
                        <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--color-info)', fontFamily: 'var(--font-head)' }}>AI Parse</span>
                    </div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                        <input type="text" placeholder='"Dinner ₹2400 split 4 ways with Raj, Priya, Sam"'
                            value={nlInput} onChange={e => setNlInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleNlParse()}
                            style={{ flex: 1, padding: '8px 12px', background: 'var(--bg-alt)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 8, fontSize: '12px', fontFamily: 'var(--font-body)', outline: 'none' }} />
                        <button type="button" onClick={handleNlParse} disabled={nlLoading || !nlInput.trim()}
                            style={{ padding: '8px 14px', background: 'var(--accent-light)', border: '1px solid var(--accent-border)', borderRadius: 8, color: 'var(--accent)', fontSize: '12px', fontWeight: 600, cursor: nlLoading || !nlInput.trim() ? 'not-allowed' : 'pointer', opacity: nlLoading || !nlInput.trim() ? 0.6 : 1, fontFamily: 'var(--font-body)', whiteSpace: 'nowrap' }}>
                            {nlLoading ? '…' : 'Parse'}
                        </button>
                    </div>
                </GCard>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        ...
                        {form.participants.map((p, i) => (
                            <div key={i} style={{ display: 'flex', gap: '8px' }}>
                                <input type="text" placeholder={`Person ${i + 1} name`} value={p.name} onChange={e => updateParticipant(i, e.target.value)} style={{ flex: 1, padding: '8px 12px', background: 'var(--bg-alt)', color: 'var(--text-primary)', border: '1px solid var(--border)', borderRadius: 8, fontSize: '13px', fontFamily: 'var(--font-body)', outline: 'none' }} />
                                {form.participants.length > 1 && (
                                    <button type="button" onClick={() => removeParticipant(i)} style={{ width: 34, height: 34, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-muted)', cursor: 'pointer' }}><X size={13} /></button>
                                )}
                            </div>
                        ))}
                    </div>
