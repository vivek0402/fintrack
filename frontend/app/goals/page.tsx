...
const outlineBtn: React.CSSProperties = { padding: '6px 12px', background: 'transparent', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', color: 'var(--text-secondary)', fontSize: '12px', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-body)', transition: 'all var(--transition-fast)', display: 'flex', alignItems: 'center', gap: 4 };
        ...
        <AppLayout>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '24px', animation: 'fadeUp 200ms ease forwards' }}>
                ...
                {goals.length > 0 && (
                    <GCard>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                            <p style={{ fontFamily: 'var(--font-head)', fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>Overall Progress</p>
                ...
                <div
                    ...
                    style={{ background: 'color-mix(in srgb, var(--color-info) 8%, var(--bg-card))', border: '1.5px solid color-mix(in srgb, var(--color-info) 20%, transparent)', borderRadius: 'var(--radius-lg)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}
                >
                    <Brain size={18} color="var(--color-info)" style={{ flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                        <p style={{ fontFamily: 'var(--font-head)', fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 2px' }}>AI Life Event Planner</p>
                ...
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                        ...
                        <p style={{ fontFamily: 'var(--font-head)', fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 8px' }}>No goals yet</p>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {goals.map(goal => {
                            ...
                            return (
                                <div key={goal.id} style={{ background: 'var(--bg-card)', border: `1px solid ${isComplete ? 'color-mix(in srgb, var(--color-inc) 20%, transparent)' : 'var(--border)'}`, borderRadius: 'var(--radius-lg)', padding: '16px' }}>
                                    {/* Top row: emoji + name + deadline | % + label */}
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px', gap: '10px' }}>
                                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', flex: 1, minWidth: 0 }}>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '3px' }}>
                                                    <span style={{ fontFamily: 'var(--font-head)', fontSize: '14px', fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                        {goal.name}
                                                    </span>
                                                    {isComplete && (
                                        ...
                                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '22px', fontWeight: 800, color: isComplete ? 'var(--color-inc)' : 'var(--accent)', margin: '0 0 1px', fontVariantNumeric: 'tabular-nums', animation: 'numberReveal 400ms cubic-bezier(0.22,1,0.36,1) both' }}>
                                                {Math.round(pct)}%
                                            </p>
                                            <p style={{ fontSize: '11px', color: 'var(--text-muted)', margin: 0, fontFamily: 'var(--font-body)' }}>saved</p>
                                        </div>
                                    </div>

                                    {/* Progress bar */}
                                    <ProgressBar pct={pct} color={isComplete ? 'var(--color-inc)' : (goal.color || 'var(--accent-2)')} height={7} />
                                        ...
                                        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                                            ...
                                            {confirmDeleteId === goal.id ? (
                                                <div style={{ display: 'flex', gap: '4px' }}>
                                                    ...
                                                    <button type="button" onClick={() => setConfirmDeleteId(null)} style={{ padding: '4px 8px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-alt)', border: '1px solid var(--border)', color: 'var(--text-secondary)', fontSize: '11px', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
                                                        Cancel
                                                    </button>
                                                </div>
                                            ) : (
            ...
            <Modal isOpen={showForm} onClose={() => { setShowForm(false); setFormError(''); }} title="New Goal" maxWidth="460px"
                footer={
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        ...
                        <button type="submit" form="new-goal-form" disabled={formLoading || !form.name.trim() || !form.target_amount} style={{ padding: 10, background: formLoading || !form.name.trim() || !form.target_amount ? 'var(--border)' : 'var(--accent)', border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontFamily: 'var(--font-body)', cursor: formLoading ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
            ...
            <Modal isOpen={!!editingId} onClose={() => { setEditingId(null); setEditError(''); }} title="Edit Goal" maxWidth="460px"
                footer={
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        <button type="button" onClick={() => { setEditingId(null); setEditError(''); }} style={{ padding: 10, background: 'var(--bg-alt)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text-secondary)', fontSize: 14, fontFamily: 'var(--font-body)', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
                        <button type="submit" form="edit-goal-form" disabled={editLoading} style={{ padding: 10, background: editLoading ? 'var(--border)' : 'var(--accent)', border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontFamily: 'var(--font-body)', cursor: editLoading ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
            ...
            <Modal isOpen={!!fundsGoalId} onClose={() => { setFundsGoalId(null); setFundsAmount(''); setFundsType('add'); }} title="Update Savings" maxWidth="360px"
                footer={
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        <button type="button" onClick={() => { setFundsGoalId(null); setFundsAmount(''); }} style={{ padding: 10, background: 'var(--bg-alt)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text-secondary)', fontSize: 14, fontFamily: 'var(--font-body)', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
                        <button type="button" onClick={handleAddFunds} disabled={fundsLoading || !fundsAmount} style={{ padding: 10, background: fundsLoading || !fundsAmount ? 'var(--border)' : 'var(--accent)', border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontFamily: 'var(--font-body)', cursor: fundsLoading ? 'not-allowed' : 'pointer', fontWeight: 600 }}>
            ...
            >
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
                    {(['add', 'withdraw'] as const).map(t => (
                        <button key={t} type="button" onClick={() => setFundsType(t)} style={{ padding: '10px', borderRadius: '10px', border: `1px solid ${fundsType === t ? (t === 'add' ? 'color-mix(in srgb, var(--color-inc) 30%, transparent)' : 'color-mix(in srgb, var(--color-exp) 30%, transparent)') : 'var(--border)'}`, background: fundsType === t ? (t === 'add' ? 'color-mix(in srgb, var(--color-inc) 10%, transparent)' : 'color-mix(in srgb, var(--color-exp) 10%, transparent)') : 'var(--bg-alt)', color: fundsType === t ? (t === 'add' ? 'var(--color-inc)' : 'var(--color-exp)') : 'var(--text-secondary)', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--font-body)', transition: 'all var(--transition-fast)' }}>
            ...
            <Modal isOpen={showLifeEvent} onClose={() => setShowLifeEvent(false)} title="Plan a Life Event" maxWidth="500px">
                {!lifeEventResult ? (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                                {[
                                    ...
                                    <button key={ev.type} type="button" onClick={() => setLifeEventForm({ ...lifeEventForm, event_type: ev.type })}
                                        style={{ padding: '12px 8px', borderRadius: 12, border: lifeEventForm.event_type === ev.type ? '2px solid var(--accent)' : '1px solid var(--border)', background: lifeEventForm.event_type === ev.type ? 'var(--accent-light)' : 'var(--bg-alt)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, transition: 'all 0.15s' }}>
                    ...
                    <div>
                        <div style={{ padding: 16, background: lifeEventResult.plan?.is_achievable ? 'color-mix(in srgb, var(--color-inc) 8%, var(--bg-card))' : 'color-mix(in srgb, var(--color-warn) 8%, var(--bg-card))', border: `1px solid ${lifeEventResult.plan?.is_achievable ? 'color-mix(in srgb, var(--color-inc) 20%, transparent)' : 'color-mix(in srgb, var(--color-warn) 20%, transparent)'}`, borderRadius: 12, marginBottom: 16 }}>
                            ...
                            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                                ...
                                <Badge color="var(--text-muted)" bg="var(--bg-alt)">{lifeEventResult.plan?.difficulty}</Badge>
                            </div>
                        </div>
                            ...
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                    {lifeEventResult.plan.milestones.map((m: any, i: number) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'var(--bg-alt)', borderRadius: 8 }}>
