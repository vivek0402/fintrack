...
| 2026-08-25 | **Bottom nav gets its own, lighter glass fill (`.glass-nav`)** | Requested more translucent than the standard `.glass-surface` recipe. Given its own modifier + `--glass-nav-surface` token rather than lowering the shared token — the nav sits over scrolling page content, not a card grid, so it can read lighter without hurting text legibility elsewhere |
