...
- No `color-mix()` inline — define a semantic token if you need a derived color.
  For translucent fills over glass the tokens already exist: **`--glass-fill-1`** (fields,
  inputs, tiles at rest), **`--glass-fill-2`** (raised chips/avatars, and the hover of
  level 1), **`--glass-fill-3`** (selected/active). Never hand-write
  `color-mix(… var(--text-primary) N%, transparent)`, and never mix a tint against an
  opaque base such as `var(--bg-surface-1)` — that bug shipped seven times before the
  tokens existed
...
| 2026-08-26 | **Nested components, group 1 of 4 — app chrome** | A post-rollout audit found 20 component files (~100 opaque hits) that the page-by-page passes never reached: the habit of grepping for child components only began at Investments/CamsImporter, so pages converted before that kept opaque children. Being cleaned up in four groups; this is the global chrome, visible on every page. **Desktop `Sidebar`** → `.glass-surface .glass-nav` — the *dense* nav fill, not the lighter card recipe, because page content scrolls directly beside it and that is the same legibility problem the mobile pill hit on 2026-08-25; its all-round class border is overridden back to a right edge only. Both sidebar hover handlers → translucent `color-mix` instead of `--bg-surface-2`. **`GlobalSearch`** → trigger chip and the ESC hint/result icon squares to `.glass-field`, the fixed results panel to `.glass-surface .glass-sheet` (it is a floating dialog, so it gets the sheet fill like every `Modal`), its three internal dividers to `--glass-border`, and row hover to a translucent tint. **`BottomNav`** → inactive More-panel tiles to `.glass-field`, active accent tile untouched. **`AppLayout` deliberately unchanged**: both its hits are the FAB hover tooltips, and tooltips have stayed opaque throughout the rollout for legibility |
| 2026-08-26 | **Nested components, group 2 of 4 — Dashboard widgets** | Dashboard was converted at the very start of the rollout, before the grep-for-nested-components habit existed, so its five child widgets kept opaque surfaces. `HealthScoreWidget` — all three states (loading, no-data, main card) → `.glass-surface`, and its hover, which swaps `borderColor` to `--accent` and back, now resets to `--glass-border`. `NetWorthWidget` — the three breakdown chips (Banks / Investments / Debt) → `.glass-field`, matching the FY chip converted on Documents; these are hand-rolled spans, not shared `Badge` components, which stay opaque. `RecentTransactions` and `SpendingForecast` — both cards were built on `--bg-surface-2` rather than `-1` (which is why they never quite matched the other widgets even before glass) → `.glass-surface`; the row hover's `--bg-surface-3` → translucent tint, and SpendingForecast's inner stat strip → `.glass-field` now that it nests inside a glass card. `WealthVelocityWidget` — **corrects an earlier call of mine**: the Net Worth entry documented leaving its "Need 2+ months of data" badge opaque as a status chip, but its three sibling entries in the same `TREND_BADGE` object all use translucent `color-mix` tints and only this one was solid. That was an internal inconsistency rather than a precedent, so it now matches its siblings |
| 2026-08-26 | **Nested components, group 3 of 4 — Transactions children** | The largest group: 59 opaque hits across 7 files (~3,100 lines). Transactions was converted very early in the rollout, so none of its children were touched. All 59 reduced to five repeating patterns: floating dropdown panels (saved-view lists, category pickers, the bulk-ops bar, `BulkOpsPanel`'s own hand-rolled `sheetStyle`) → the glass-sheet fill, consistent with `GlobalSearch`'s panel and the Accounts/Groups modal-shell precedent; form fields and inputs → `.glass-field`; inactive chips and toggle pills → `.glass-field` with active accent states untouched; row hovers → translucent `color-mix`. The fifth pattern was a small behavioural improvement rather than a straight swap: **multi-select rows filled with neutral `--bg-surface-2`**, which reads as a flat grey patch on glass, so selection now uses an accent-tinted wash that also communicates the state better. One genuine oddity handled separately: `AdvancedSearchBar`'s amount range-slider draws its thumb ring with `border: 2px solid var(--bg-surface-1)` inside a raw CSS `<style>` string, not inline styles — that ring exists to punch a visual gap against the track, so it must match whatever sits *behind* the panel and became `--bg-base`, not a glass token, since a translucent ring would defeat the purpose |
| 2026-08-26 | **Nested components, group 4 of 4 — stragglers; the cleanup is complete** | 18 hits across 4 files, of which **13 converted and 5 were deliberate holds**. `BankAccountsSection` (Profile was page 1 of the rollout, so this child was never touched): top-level card → `.glass-surface`; its hand-rolled modal → `.glass-surface .glass-sheet` and the account-actions dropdown → the same sheet fill inline, both per the Accounts/Groups precedent; account rows, skeleton tiles, icon-picker swatches and the two shared style consts → `.glass-field`; the default-account toggle's off-state track → a translucent neutral instead of `--bg-surface-3`. `CoachAlerts` — its three `color-mix(…, var(--bg-surface-1))` bases → `transparent`, the fourth and last instance of that opaque-base pattern (after SuggestionsBanner, Detect-Patterns and AI Parse). `CategoryTrajectory` — outer category tile → `.glass-field`; its other hit is a recharts tooltip and stays. **`SpendingHeatmap` left entirely untouched**, and the reason is worth recording: its `--bg-surface-2` is not a surface but the *base of the data colour scale* — `cellColor()` returns it for a zero-spend day and uses it as the `color-mix` base for every intensity step. Translucency would let the ambient backdrop bleed through, so identical spend would read as different intensity depending on screen position; the encoding would lie. Same principle that has kept every chart tooltip opaque |
| 2026-08-26 | **Correction: the group-4 entry above was wrong to call the cleanup complete** | That entry and its commit both said the nested-component cleanup was finished. It was not. The four groups were scoped by *feature* subdirectory (analytics, dashboard, transactions, layout, profile, coach) and `components/ui/` — the shared component directory — was never enumerated at all, nor were the page files re-swept after their own conversions. Roughly 56 further hits remained. Recording the mis-scope here rather than quietly fixing it, since "the rollout is done" has now been claimed prematurely twice and the log should show that |
| 2026-08-26 | **Nested components, group 5 — shared `components/ui/`** | ~21 hits across 9 files, all app-wide. Floating overlays (`NotificationBell`'s dropdown, `PageHelp`'s panel, `DatePicker`'s calendar popover, `WalkthroughTour`'s card, `RedesignAnnouncement`'s dialog) → the glass-sheet fill, matching `Modal` and `GlobalSearch`; their triggers, inner rows and segmented controls → `.glass-field`; hovers → translucent tints. Two more `color-mix(…, var(--bg-surface-1))` opaque bases fixed (`AIResponseCard`, `NotificationBell`) — the fifth and sixth instances of that pattern. **Two shared-component defaults flipped**, both the same "the rationale expired" situation as `Card`/`GCard`/`StatTile`: `Input`'s `variant="glass"` prop was added during the auth-screen pass because most pages were still opaque, but 8 of 18 call sites were passing it explicitly and the other 10 sat stale on glass pages — the glass recipe is now the default and the branch is gone; and `Button`'s `secondary` and `icon` variants (27 + uses) moved from `--bg-surface-2`/`-3` to `.glass-field` with a translucent hover, which is exactly the treatment every hand-rolled Cancel button already received during the rollout, so the shared component finally matches its own hand-rolled equivalents. `ghost` and `danger` were already translucent. **`Skeleton` deliberately untouched**: its one hit is the shimmer gradient, whose effect depends on solid stops sliding past each other — translucent stops would let the backdrop through and the sweep would stop reading as a sweep, same category as the heatmap scale and chart tooltips |
| 2026-08-26 | **Nested components, group 6 — page-file re-sweep; every remaining opaque surface is now a recorded hold** | The pages were converted one at a time but never re-swept afterwards, leaving 42 hits. Two real findings. **A seventh instance of the `color-mix`-against-an-opaque-base defect**, this time on Dashboard's KPI tiles (`color-mix(in srgb, ${tile.color} 10%, var(--bg-surface-1))`) — each tile was punching a solid rectangle through the glass; Dashboard predates the systematic process, which is why it kept the original form. **Dead code removed**: `analytics/page.tsx`'s `readChartColors()` resolver declared a `bgCard` field on `ChartColors`, gave it a `DEFAULT_CC` default and populated it on every call, but nothing in the file ever read it — the field is gone rather than converted, which was the sole reason two of Analytics' hits existed. The rest was routine: five neutral `Badge` `bg` overrides → translucent (the shared `Badge` already defaults to translucent `--accent-subtle`, so the opaque override was the outlier); three status-config maps where the neutral entry was solid among translucent siblings, same shape as the `WealthVelocityWidget` correction; and Profile's fields/icon tiles/theme picker, Transactions' empty-state cards, Dashboard's period pills and progress tracks, Health Score, Register and the loading-skeleton blocks. **Every `--bg-surface-*` reference still in `app/` and `components/` is now a deliberate hold with a reason recorded in this log**: the nine chart tooltips, `AppLayout`'s two FAB tooltips, the Accounts and Analytics toasts, disabled-button fills, `Skeleton`'s shimmer gradient, and `SpendingHeatmap`'s data colour scale |
| 2026-08-26 | **Glass fill scale added (`--glass-fill-1/2/3`); 161 inlined `color-mix` calls tokenized** | Post-rollout audit against DESIGN.md's own anti-pattern rule — *"No `color-mix()` inline — define a semantic token"* — found the rule broken 159 times, almost entirely by this rollout: components that accept `style` but not `className` cannot use `.glass-field`, so the recipe was retyped by hand rather than named. Worse, the opacity had drifted across **nine** values (3, 4, 5, 5.5, 6, 8, 10, 12, 14%) with nothing to anchor them. Those nine cluster into three roles, so the new tokens are named after the existing `--bg-surface-1/2/3` scale to keep one mental model across both systems: `--glass-fill-1` (5%) fields/inputs/tiles at rest, `--glass-fill-2` (9%) raised chips and avatars *and* the hover of level 1, `--glass-fill-3` (13%) selected/active. `.glass-field` now consumes `--glass-fill-1` rather than repeating the value, so class and token cannot drift apart. All 161 call sites converted across 46 files; zero hand-written instances remain. The real payoff is not tidiness: **the `color-mix`-against-an-opaque-base defect appeared seven separate times** during this work, every instance someone writing `color-mix(…, var(--bg-surface-1))` because the correct base had no name — the anti-pattern entry now names both the right tokens and that specific wrong turn |
| 2026-08-26 | **Test coverage for the shared UI primitives (8 → 34 tests); dead `Input` `variant` prop removed** | Companion to the token work, prompted by an honest accounting of what "verified" meant across this rollout: 51 `.tsx` files changed with **two** frontend test files covering none of them, versus 28 on the backend. Every verification was `tsc` + `next build` — proof the code compiles and renders, not that it behaves. Added `Card.test.tsx` (Card/GCard/StatTile) and `Button.test.tsx` (Button/Input/Tabs) and `Modal.test.tsx`, 26 new tests asserting the contracts this rollout actually depends on: that the glass look comes from the *class* so callers need no override, that a caller's `style` prop still wins over it (the property that let 12 duplicated `glassTileStyle` consts be deleted), that no inline `background` shadows the class fill, that hover lifts exactly one fill level and clears back to the class on leave, and that `Modal`'s scroll lock is genuinely ref-counted so a stacked date-picker modal cannot unlock scrolling while its parent is still open. Verified the suite can actually fail: reverting `Card` to an opaque fill breaks 4 of them. Two jsdom notes recorded in the tests themselves — `toHaveStyle` cannot parse a `border` shorthand containing `var()` (the style *attribute* keeps it, so assert there), and `StatTile`'s loading branch is a separate return path that has regressed independently before. Also removed `Input`'s `variant` prop: flipping the default on 2026-08-26 left it declared, destructured and documented but never read — dead in the same way `bgCard` was |
| 2026-08-26 | **Logic coverage for `lib/` (34 → 76 tests)** | Second half of closing the verification gap. Rather than shallow render tests for all 34 pages, this targets the pure logic those pages depend on, where a bug corrupts numbers silently instead of crashing. **`lib/utils.ts`** — the three money classifiers (`isNonSavingsExpense`, `isRealIncome`, `isCategorizableExpense`) that decide what counts as real spending, real income and category-attributable spending; they feed savings rate and most of Analytics. Note their header comment says they mirror `backend/src/utils/savingsRate.js` and must be kept in sync — the two were diffed and confirmed identical, but nothing enforces it beyond that comment, so these cases at least catch frontend-side drift. The one difference between the two expense predicates (`isCategorizableExpense` deliberately *keeps* investment-category rows so an "Investments" line can appear in by-category charts) is pinned explicitly. Also covers `formatDate`'s string-split parsing, which exists so a UTC-midnight timestamp cannot render as the previous day for users behind UTC. **`lib/transactionFilters.ts`** — the five exports that had no coverage, including `sortTransactions`, added during the Transactions redesign and shipped untested; its amount comparisons are pinned as numeric because the API returns amounts as strings and a lexicographic sort would rank `'9'` above `'120'`. **`lib/healthScore.ts`** — the eight-factor scorer, previously untested: the maxima are asserted to total exactly 100, no factor may exceed its own max, and empty history must not leak `NaN` onto the gauge (`mean([])` is `0/0`, so the short-array guards are what prevent it). One behaviour pinned rather than changed: `fmt(-500)` renders `₹-500`, not the conventional `-₹500`, because the symbol is concatenated ahead of `toLocaleString`'s sign — 22 call sites already pass `Math.abs()` and supply their own prefix, so "fixing" it would alter output where no sign is expected |
| 2026-08-26 | **Integration coverage for the add-transaction flow (76 → 82 tests)** | Closes the last named gap: the primitives and pure logic were covered, but nothing exercised a composed flow. Six tests over `TransactionModal`, the app's highest-traffic write path, asserting what actually reaches the API rather than what renders — payload contents, the expense/income switch, the duplicate guard (a near-identical recent transaction is held back rather than written; a clearly different amount passes), and the transfer double-write. That last one is the most valuable: **a transfer is not one row with a type, it is a matched expense/income pair, and the `transfer` tag on both is what keeps them out of real spending and income** via `isNonSavingsExpense`/`isRealIncome` — so a regression there would silently distort savings rate rather than break anything visibly. Verified the suite catches it: deleting the income half of the double-write fails the test. Two findings while writing these. **A guard that the UI makes unreachable** — `handleSubmit` rejects a transfer whose From and To match, but the To dropdown is built from accounts *minus* whichever From holds, so that branch cannot be driven by a user; the test now asserts the reachable behaviour (the source account is absent from the destination list) rather than the dead branch. **A jsdom limitation worth knowing before writing more component tests**: Testing Library's `getAllByRole` clones nodes internally, and jsdom's CSS shorthand parser throws while re-parsing a `background` shorthand containing `color-mix()`. Since the glass tokens made `color-mix` ubiquitous, role queries can now crash on some subtrees — query the DOM directly there. Recorded in the test file itself |
| 2026-08-26 | **Integration coverage for the filter flow (82 → 90 tests); frontend suite now 8 → 90** | The read-path counterpart to the add-transaction tests. `applyAdvancedFilters`/`countActiveFilters` were already unit-tested, so these eight assert the *wiring*: that driving the real controls produces the right set on `onFilter` — free-text search matching description and category case-insensitively, clearing restoring the full list, the type pills narrowing and widening again, and the active-filter count being reported so an external top bar can badge it. The most load-bearing case is **date context**: choosing "All time" or "Custom range" needs data the page has not fetched, since it pages by month, so the component must call `onSetDateContext` — without it the filter would silently apply to an incomplete set and quietly show the user the wrong answer. Verified the suite fails on a real regression: deleting the type-filter line breaks 3 of the 8. Worth recording that the *first* sabotage attempt passed — the string I tried to remove did not exist in that shape, so the check proved nothing until it was retried against the actual line; a regression check that does not visibly fail has not verified anything. This closes the testing work: the frontend went from **8 tests over 2 files to 90 over 10**, covering the shared primitives, the pure logic (money classifiers, filters, health score) and both high-traffic flows |
| 2026-08-26 | **Page-level composition tests for Transactions (90 → 97); frontend suite 8 → 97** | The last uncovered layer. Rather than thin render tests across all 34 routes, this covers the one page that carries logic existing *only* at page level, on the app's busiest route. Seven tests over `app/transactions/page.tsx`: that it pages by month rather than pulling all history, that results render and are written to the offline cache, and — the reason this layer was worth testing at all — **the offline-cache fallback**. That path is deliberately not routed through `apiWithCache` (which swallows failures silently) so a failing backend stays distinguishable from an empty month: on error the page falls back to cached rows, and raises a visible error state *only* when the cache is also empty, since an empty list would otherwise read as "no transactions this month". All three of those states are pinned. Verified against a real regression: bypassing the cache fallback fails 2 of the 7. One behaviour discovered while writing them and worth recording — a `?credit_card_id=` deep link (from the Accounts page's per-card History button) does two things at once: it coerces the id to a number *and* clears the month pager, because a card's history is only useful whole; scoping to a card while still showing a single month would usually look empty. That pairing was a comment in the source and is now an assertion. **This completes the testing work begun after the rollout: 8 tests over 2 files → 97 over 11**, spanning shared primitives, pure logic, both high-traffic flows, and page composition |
| 2026-08-26 | **Routing-contract tests (97 → 103); the remaining 33 pages deliberately left without page tests** | Asked to cover the rest of the pages, the useful answer turned out not to be 33 thin render tests. An audit for page-*unique* logic found that Transactions is the only page with an offline/error path — the others' only page-level logic is `?tab=` deep-link resolution. That, however, hides a real unenforced contract: **eleven routes are thin `redirect()` stubs pointing at a tabbed page with a `?tab=` value** (`/recurring` → `/budgets?tab=recurring`, `/loans` → `/debt-intelligence?tab=loans`, and so on), and each target validates that value against its own key list, *silently falling back to its default tab* when it does not match. Renaming a tab key therefore throws nothing, fails no build and passes type-checking — the user clicks "Recurring" in the nav and quietly lands on "Budgets". Six tests now read the real source at test time (rather than restating the routes, so they stay true as routes change) and assert: every stub targets a route that exists, every `?tab=` value is one its target actually accepts, every tabbed page guards the URL value rather than trusting it, and the stubs stay thin. Verified by renaming `budgets`' `recurring` key — the failure message names the stub, the target and the user-visible consequence. One near-miss worth recording: a first pass flagged `reports`, `year-review` and `personality` as broken, because Analytics keeps those three in a separate `EXTRA_TABS` string array (they are deep-link-only, with no pill) rather than in its `OUTER_TABS` objects — the parser had to account for both before the result meant anything. **This is the stopping point for the testing work: 8 tests over 2 files → 103 over 12.** The other 33 routes compose primitives and logic that are now covered beneath them; the marginal value of a render test per page is low, and a page earning one (as Transactions did, via its cache fallback) should get it when that logic appears |
| 2026-08-26 | **Backend: the untested financial utils covered (170 → 209 tests)** | The backend already had 28 suites over its routes, so the gap was in `src/utils/` — pure financial maths, none of it covered. Three files, chosen because a bug in each is silent rather than loud. **`savingsRate.js`** closes the cross-stack contract flagged earlier: its `isNonSavingsExpense`/`isRealIncome` are hand-mirrored in `frontend/lib/utils.ts`, reached by different code paths (SQL aggregation here, JS reduction there), so a drift would surface only as the API and the UI disagreeing about the same month. The case table is deliberately identical to the frontend suite's, so a one-sided edit fails one of them; three further tests read the frontend source directly and assert both predicates still exist, still exclude the same tags, and still carry the sync note — the note being part of the contract, since without it nobody knows to keep the copies in step. Also pins the SQL twin's shape: it must keep the `type = 'expense' AND` guard, or the investment/goal exclusions would drop income rows and understate income. **`amortization.js`** — EMI cross-checked against the closed-form formula, the zero-rate branch that exists because the formula divides by `(factor − 1)`, the invariant that interest + principal reconciles to the instalment on every row, and the guard rejecting an EMI at or below the monthly interest (without it the balance grows forever and the loop only stops at the 1200-month cap, quoting a 100-year payoff as real). **`creditCardBalance.js`** — a card with no billing date must degrade to nulls rather than invent a statement balance, the last close may never be dated in the future, statement plus new charges must reconcile to the current balance, and a null balance must sum to zero rather than `NaN`, which would otherwise poison every net-worth figure derived from it. All three verified by injected regressions: dropping the `credit_card_payment` exclusion, removing the EMI guard, and removing the null-coalesce fail 4 tests between them |
| 2026-08-26 | **Backend: validation + the largest untested route (209 → 257 tests)** | `validation.js` guards every route's input and had no coverage. Most of it is allow-lists, but the numeric and date checks lean on `parseFloat` and `new Date`, both lenient in ways that decide whether bad input reaches the database — so the leniency is pinned rather than left implicit: `isPositiveNumber('250abc')` is **true** (parseFloat stops at the number), and `isValidDateString('2026-02-30')` is **true** (Date rolls it to 2 March), meaning that validator guarantees shape and parseability, not calendar correctness. `'1e999'` is rejected only because `Number.isFinite` catches the overflow to `Infinity`. Also pinned: `TRANSACTION_TYPES` is exactly `['income','expense']` and rejects `'transfer'`, because transfers are stored as a matched pair tagged `transfer`, never as a third type. `analytics.js` was the largest untested route file (491 lines, 10 endpoints); the tests concentrate on `/wealth-velocity`, which carries the most derived logic and whose `trend` is what the dashboard's `WealthVelocityWidget` renders — covering the two-snapshot and six-snapshot thresholds, the accelerate/decelerate/steady bands, the divide-by-zero guard when a new user's first snapshot is `0`, and the use of `Math.abs` on the denominator so a user in net debt improving from −100k to −50k reads as +50% rather than −50%. A whole-file check asserts every listed endpoint filters by `user_id`. Two process notes: the suites were verified by injected regressions (removing the divide-by-zero guard and the `Number.isFinite` check fails 2), and one of my own tests initially failed for a bogus reason — I had written `expect(value, message)`, which is vitest syntax carried over from the frontend work and which Jest rejects outright; the assertion was rewritten to collect offenders and compare by value, since Jest has no custom-message argument |
| 2026-08-26 | **Backend: `/peer-benchmarks` covered (257 → 272 tests)** | `insights.js` compares the user's spending mix against income-bracket bands. Its two helpers — `mapCategoryToGroup` (keyword taxonomy) and `getIncomeBracket` — are module-local, so they are exercised through the route, which also covers its own arithmetic: bracketing on a *three-month average* rather than the raw window total (without the division every user would be bracketed three times too high), keyword matching that is case-insensitive and substring-based, an unmatched category being ignored rather than folded into a group, band statuses, deviation from band midpoint, and the divide-by-zero guard for a month with no income. The test that matters most pins a **deliberate two-query design the source comments at length**: the by-category query still counts investing, so the "Savings & Investments" comparison bucket is not empty for anyone using a real Investments category, while the savings-rate figure excludes it. Collapsing them — the obvious "simplification" — would silently understate every user's savings rate; doing exactly that fails 11 of the 15 tests. One setup note worth recording for the next backend route test: `insights.js` transitively requires `utils/ai.js`, which constructs a Groq client *at require time* and throws when `GROQ_API_KEY` is absent, so the suite fails to load rather than failing an assertion. A dummy key at the top of the file is the fix, matching `tests/utils.ai.test.js` |
| 2026-08-26 | **Backend: AI Advisor conversation CRUD covered (272 → 284 tests)** | `agents.js` is mostly prompt construction, but three of its routes decide who can read and delete whose chat history — and they are shaped differently from the rest of the codebase. Where other routes scope with `AND user_id = $2` in SQL, `GET /conversations/:id` and `DELETE /conversations/:id` fetch the row **by id alone and compare `user_id` in JavaScript**. That works, but it means the ownership check is a single `if` with no SQL-level backstop: delete the line and another user's conversation is served with a 200. Twelve tests now act as that backstop — 200/403/404 kept distinct (a 404 for someone else's row would leak whether an id exists; a 403 for a missing one would mislead), another user's content asserted absent from the 403 body since the row is fetched *before* the check, and `agent_type` validated before any query runs. The sharpest one covers DELETE: it asserts not merely the 403 but that **no `DELETE` statement is issued at all**, because that statement is itself unscoped — reaching it would remove another user's data. Verified by removing both ownership checks: 3 tests fail. **This closes the testing work across both stacks — frontend 103 over 12, backend 284 over 35: 387 tests, up from 178.** What remains untested is the AI orchestration itself (prompt builders, model calls, the 60-second unified-data cache), where the deterministic surface is thin and the value drops off sharply |
| 2026-09-16 | **Desktop sidebar split from the mobile nav's glass density (`--glass-sidebar-surface` added)** | User feedback on a screenshot: the sidebar read as flat, not glass, even though it already carried `.glass-surface .glass-nav`. That fill (`--glass-nav-surface`, 0.88 dark / 0.90 light) was two separately-made legibility calls — mobile pill (2026-08-25), then desktop sidebar reusing the same token (2026-08-26) — both reverted from a lighter treatment after busy scrolling content made it unreadable. Rather than re-litigate blind, five candidates (today's fill, plus four variants — boosted blur/edge at the same opacity, a fully translucent fill, a middle-ground opacity, and a hybrid) were mocked side by side over the real ambient backdrop and a busy transaction list. **Middle ground was chosen** for the desktop sidebar only: `--glass-sidebar-surface: rgba(10,10,10,0.55)` dark / `rgba(255,255,255,0.65)` light, applied via a new `.glass-nav-desktop` modifier (wins over `.glass-nav` by source order, same convention as `.glass-nav` over `.glass-surface`) so the mobile `BottomNav` pill keeps its own separately-tuned 0.88/0.90 density untouched — explicitly scoped to desktop only, since the mobile pill's density was its own tuned decision with the identical failure history and wasn't up for re-litigation here |
| 2026-09-16 | **`EmptyState` icon gets a continuous idle float + sparkle loop, an intentional exception to "only comprehension-aiding motion"** | Part of a small motion-polish batch. Approved via a user-reviewed HTML/CSS prototype rather than designed fresh in code. This is the first continuous/infinite loop added to ordinary page content that is purely decorative — the Motion section's default ("only transitions that aid comprehension or signal state") does not cover it, so it's recorded here as a deliberate, scoped exception rather than a silent departure. Justification for the exception: low-amplitude (±4px float, brief sub-second sparkle pulse per 2.4s cycle) and sitting on an already-muted `--text-muted` icon, so it reads as ambient texture rather than an attention-grabbing effect; respects `prefers-reduced-motion` (unlike loading spinners, which are informational and stay exempt). New `animated?: boolean` prop (default `true`) is the escape hatch for call sites where the loop doesn't fit — `EmptyState` is also used for genuine error/failed-load states (e.g. `app/personal-loans/page.tsx`'s "Couldn't load your personal loans"), where an idly floating, sparkling icon reads wrong; that one site was switched to `animated={false}`, other call sites left on the default. Sparkle uses `var(--accent)`, not `var(--color-warn)` — `--color-warn` is the app's semantic warning/alert token used across ~30 files (BudgetAlerts, CoachAlerts, DTI widgets) and borrowing it for a permanent decorative accent would be a semantic mismatch |
| 2026-09-16 | **Desktop sidebar made collapsible, closing a doc/implementation gap** | This section already documented the desktop sidebar as "collapsible" since the original v1 layout spec, but `Sidebar.tsx` never implemented it — fixed 240px, no toggle. Added a collapsed state (76px, icon-only) toggled by a small circular button floating on the sidebar's right border (`PanelLeftClose`/`PanelLeftOpen`), chosen over cramming a toggle into the header row so it doesn't compete with the wordmark/bell at either width. Collapsed state hides labels (nav items get a `title` tooltip instead), the search trigger (Ctrl+K still works globally), the tour button, notification bell, user name/email, and `InstallPWA`, keeping just icons, the avatar, and the More toggle. State lives in `themeStore` (`sidebarCollapsed`, persisted to `localStorage` as `fintrack-sidebar-collapsed`) so `AppLayout`'s content margin can react too — this also repurposed that store's `sidebarWidth`/`setSidebarWidth` fields, which had no consumers anywhere and still held a stale 220px default against the sidebar's real 240px |
