...
- **Known deliberate scope decisions (not oversights):** the new client-side caches use TTL expiry only, no write-triggered invalidation (unlike the existing `dashboard-cache-*` pattern) — accepted staleness window of up to 30 minutes for slow-moving data (investment ratio, salary intelligence) in exchange for not having to wire invalidation into every account/investment/loan write path; `opportunityAPI.getAll()` and `briefingAPI.getLatest()` are deliberately left uncached (see Task 5's inline comment for why); the connection pool size is investigated, not changed, in Task 6, since changing it without knowing Supabase's actual ceiling could make things worse, not better.
